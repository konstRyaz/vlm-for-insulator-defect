...
Гибридная ветка полезна потому, что отделяет `coarse_class` от генерации отчётных полей. DINOv2-классификатор улучшает coarse decision, а Qwen сохраняет структурированный `vlm_labels_v1`-совместимый вывод.